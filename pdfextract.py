...
def main():
    ...
    for i in args.pdf_colon_ops:
        ...
        try:
            reader = PyPDF2.PdfFileReader(pdf, warndest=None if args.verbose else sys.stdin)
        except:
            print('could not read: {}'.format(pdf))
            if args.verbose:
                print(get_cur_error())
            exit() # you can just return, since you are in the main function
            ...
            try:
                [
                    [
                        ...
                        ==
                        (print('adding page {} from {}'.format(i + 1, pdf)) if args.verbose else None)
    ...
    sys.setrecursionlimit(4 * sys.getrecursionlimit()) # let's take some risk
